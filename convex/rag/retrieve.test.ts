...
describe("retrieve.topVerses", () => {
  it("devuelve el versículo cuando la similitud supera el umbral", async () => {
    ...
    const results = await t.action(api.rag.retrieve.topVerses, { query: "¿quién es mi pastor?" });
  ...
  it("no devuelve nada cuando la similitud queda por debajo del umbral — nunca fabrica una cita", async () => {
    ...
    const t = convexTest(schema, modules);
    await seedVerse(t, unitVector(0));
    stubVoyage(unitVector(1)); // ortogonal al versículo indexado

    const results = await t.action(api.rag.retrieve.topVerses, { query: "algo sin relación" });
  ...
  it("filtra por versión", async () => {
    ...
    const results = await t.action(api.rag.retrieve.topVerses, { query: "mi pastor", version: "RVR1960" });
